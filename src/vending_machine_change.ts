...
function get_coin_inventory(coin_inventory: number){   //引数coin_inventoryの残り枚数を返す関数
    let five_hundred: number = 3;      //500yen : 3枚
    let hundred: number = 5;           //100yen : 5枚
    let fifty: number = 10;             // 50yen : 10枚
    let ten: number = 50;               // 10yen : 50枚
...
function ft_change(price: number, paid: number){
    let change: number;                                    //お釣りを格納する変数 (paid - price)
    let coin: number[] = [500, 100, 50, 10];               //お釣りで使用される硬貨[500yen, 100yen, 50yen, 10yen]
    let sheets: number[] = [0, 0, 0, 0];                   //お釣りで出てくる各硬貨の枚数[500yen, 100yen, 50yen, 10yen](初期値として0を代入)
    let i: number = 0;                                     //while loopを回す際に使用する変数
    let error_message: string = "Error : Lack of change";  //お釣りが不足している際に表示するエラーメッセージ
